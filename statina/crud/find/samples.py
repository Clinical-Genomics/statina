...
def get_sample_text_query(text: str) -> dict:
    return {
        "$or": [
...
def query_samples(
    ...
    raw_samples: Iterable[dict] = (
        adapter.sample_collection.find(get_sample_text_query(text=text))
...
def count_query_samples(adapter: StatinaAdapter, text: Optional[str] = "") -> int:
    """Count all queried samples in sample collection"""
    return adapter.sample_collection.count_documents(filter=get_sample_text_query(text=text))
...
def query_batch_samples(
    ...
    raw_samples: Iterable[dict] = (
        adapter.sample_collection.find(
            {
                "$and": [{"batch_id": batch_id}, get_sample_text_query(text=text)],
            }
        )
        .sort(sort_key, sort_table.get(sort_direction))
        .skip(skip)
...
def count_query_batch_samples(
    adapter: StatinaAdapter, batch_id: str, text: Optional[str] = ""
) -> int:
